...
const int TileMap::getLayerSize(const int x, const int y, const int layer) const
{
	if (x >= 0 && x < static_cast<int>(this->map.size()))
	{
		if (y >= 0 && y < static_cast<int>(this->map[x].size()))
		{
			...
			{
				return int(this->map[x][y][layer].size());
			}
		}
	}
	return -1;
...
{
	/*Take three indicles from the mouse position in the grid and add a tile to that position if the internal array allows it */
...
{
	/*Take three indicles from the mouse position in the grid and remove a tile to that position if the internal array allows it */
...
{
	/*Saves the entire tilemao to a text-file.
	Format:
	Basic:
	size x y
	gridSize
	...
	for (int x = this->fromX; x < this->toX; x++)
	{
		for (int y = this->fromY; y < this->toY; y++)
		{
			for (size_t k = 0; k < this->map[x][y][this->layer].size(); k++)
			{
				...
				sf::FloatRect nextPositonBounds = entity->getNextPositionBounds(dt);

				if (this->map[x][y][this->layer][k]->getCollision() &&
					this->map[x][y][this->layer][k]->intersects(nextPositonBounds)
					)
				{
					//Bottom collision
